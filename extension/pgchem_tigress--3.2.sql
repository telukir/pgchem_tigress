 ...
 *
 * Copyright (c) 2004,2014 by Ernst-G. Schmid
 *
...
'MODULE_PATHNAME', 'pgchem_isotope_pattern'
  LANGUAGE c IMMUTABLE STRICT
  COST 1
  ROWS 1000;
...
EXECUTE 'COPY (SELECT ''#Comments after SMARTS'' UNION SELECT * from (SELECT "SMARTS" FROM '||t_h_name||') t) TO '||quote_literal('/usr/local/share/openbabel/2.3.2/tigress/obdata/dictionary.txt');
